...
void OperationsTrackerService::startPhase(const std::string& phase, const std::string& resourceType, size_t total) {
    ...
    oss << "{\"event\":\"phase_start\",\"ts\":\"" << nowIso8601() << "\","
        "\"phase\":\"" << phase << "\",\"resourceType\":\"" << resourceType << "\",\"total\":" << total << "}";
    writeJsonlUnsafe(oss.str());
}
...
void OperationsTrackerService::endPhase(const std::string& phase, const std::string& resourceType, bool allSucceeded) {
    ...
    ensureLedgerOpen();
    std::ostringstream oss;
    oss << "{\"event\":\"phase_end\",\"ts\":\"" << nowIso8601() << "\","
        "\"phase\":\"" << phase << "\",\"resourceType\":\"" << resourceType << "\",\"allSucceeded\":" << (allSucceeded ? "true" : "false") << "}";
    writeJsonlUnsafe(oss.str());
    if (allSucceeded) {
        try {
...
void OperationsTrackerService::markStarted(const std::string& phase,
    ...
    ensureLedgerOpen();
    std::ostringstream oss;
    oss << "{\"event\":\"start\",\"ts\":\"" << nowIso8601() << "\",";
    oss << "\"phase\":\"" << phase << "\",\"resourceType\":\"" << resourceType << "\",\"resourceName\":\"" << resourceName << "\",";
    oss << "\"fp\":{\"configHash\":\"" << fp.configHash << "\",\"opVersion\":\"" << fp.opVersion << "\",\"bifIndex\":" << fp.bifIndex << ",\"keyLocator\":" << fp.keyLocator << ",\"size\":" << fp.size << ",\"sourcePath\":\"" << fp.sourcePath << "\",\"mtime\":" << fp.mtime << ",\"overrideSize\":" << fp.overrideSize << "}}";
    writeJsonlUnsafe(oss.str());
...
void OperationsTrackerService::markCompleted(const std::string& phase,
    ...
    ensureLedgerOpen();
    std::ostringstream oss;
    oss << "{\"event\":\"end\",\"ts\":\"" << nowIso8601() << "\","
        "\"phase\":\"" << phase << "\",\"resourceType\":\"" << resourceType << "\",\"resourceName\":\"" << resourceName << "\",\"success\":" << (success ? "true" : "false") << ",\"outputs\":[";
