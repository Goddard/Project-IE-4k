...
// Height Map (HT) - 4-bit values representing elevation (-7 to +7)
struct HeightMap {
    uint32_t width;
    uint32_t height;
    ...
    HeightMap(uint32_t w, uint32_t h) : width(w), height(h), data((w * h + 1) / 2, 0) {}
    ...
    // Get elevation value (-7 to +7) at position (x, y)
    int getElevation(uint32_t x, uint32_t y) const {
        uint8_t val = getValue(x, y);
        return static_cast<int>(val) - 7; // Convert 0-15 to -7 to +7
    }
    
    // Set elevation value (-7 to +7) at position (x, y)
    void setElevation(uint32_t x, uint32_t y, int elevation) {
        int clamped = elevation + 7;
        if (clamped < 0) clamped = 0;
        if (clamped > 15) clamped = 15;
        uint8_t val = static_cast<uint8_t>(clamped);
        setValue(x, y, val);
    }
    ...
    std::vector<uint8_t> serialize() const {
        ...
        // Color Palette (16 colors in BGRA format - grayscale for height)
        for (int i = 0; i < 16; i++) {
            uint8_t intensity = i * 17; // 0, 17, 34, ..., 255
            uint8_t b = intensity; // Blue component
            uint8_t g = intensity; // Green component
            uint8_t r = intensity; // Red component
    ...
    bool deserialize(const std::vector<uint8_t>& bmpData) {
        ...
        if (bitCount == 4) {
            ...
            // 8-bit format - convert to 4-bit by scaling down
            uint32_t rowSize = width; // 8-bit unpacked
            uint32_t paddedRowSize = ((rowSize + 3) / 4) * 4; // Pad to 4-byte boundary
            uint32_t dataSize = paddedRowSize * height; // Total data size with padding
            ...
            data.clear();
            data.resize((width * height + 1) / 2, 0); // Pre-allocate and zero-fill
            ...
            for (int32_t y = 0; y < height; y++) {
                ...
                // Convert 8-bit values to 4-bit by scaling down
                for (uint32_t x = 0; x < width; x++) {
                    uint8_t pixelValue = bmpData[rowOffset + x];
                    uint8_t scaledValue = (pixelValue * 15) / 255; // Scale 0-255 to 0-15
                    
                    // Pack into 4-bit format
                    size_t byteIndex = (y * width + x) / 2;
                    if (byteIndex < data.size()) {
                        if (x % 2 == 0) {
                            // First 4 bits of byte
                            data[byteIndex] = (data[byteIndex] & 0xF0) | (scaledValue & 0x0F);
                        } else {
                            // Second 4 bits of byte
                            data[byteIndex] = (data[byteIndex] & 0x0F) | ((scaledValue & 0x0F) << 4);
                        }
                    }
                }
            }
    ...
    // Upscale the height map by the given factor using nearest-neighbor
