...
// Light Map (LM) - 8-bit values representing lighting with palette
struct LightMap {
    uint32_t width;
    uint32_t height;
    ...
    LightMap() : width(0), height(0) {}
    LightMap(uint32_t w, uint32_t h) : width(w), height(h), data(w * h, 0), palette(256, 0) {}
    ...
    // Upscale the light map by the given factor using nearest-neighbor
